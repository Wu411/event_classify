...
def attention(center,group_label_bert,maxlen):
    query = tf.convert_to_tensor(np.asarray(center).reshape(1, 1, 768))
    #print(query)
    key_list = tf.convert_to_tensor(np.asarray(group_label_bert).reshape(-1, maxlen, 768))
    #print(key_list)
    result = tf.keras.layers.Attention()([query, key_list])

    '''scores = tf.matmul(tf.cast(query,tf.float32), key_list, transpose_b=True)
...
def attention_get_bert(clusters_center,group_label_dict,group_bert,flag,maxlen):
    if flag==True:
        ...
        for center in clusters_center.values():
            result=attention(center,group_bert,maxlen)
            with tf.Session() as sess:
                result=result.eval()
            groups_result,simi=cul_simlarity(center,group_label_dict,result)
        ...
        for noise in clusters_center:
            result = attention(noise, group_bert, maxlen)
            with tf.Session() as sess:
                result = result.eval()
...
def cul_simlarity(center,group_label,group_bert):
    ...
    score=[]
    for i in group_bert:
        s = similarity.cosSim(center, i[0])[0]
        score.append(s)
    max_score = max(score)
    simi = max_score
    for i, x in enumerate(score):
